...
describe("orderService.deleteOrder", () => {
  ...
  test("없는 주문일 때 `주문이 없습니다.` 에러 반환", async() => {
    ...
    try{
      await orderService.deleteOrder(payloadId);
    } catch (e) {
  ...
  test("배송이 pending 상태가 아닐 때 `배송 중이거나 완료된 건입니다.` 에러 반환", async () => {
    try{
      await orderService.deleteOrder(err_orderId);
    } catch (e) {
      expect(e.name).toBe(commonErrors.businessError);
      expect(e.message).toBe("배송 중이거나 완료된 건입니다.");
